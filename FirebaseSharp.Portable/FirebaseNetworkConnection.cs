			...
			try {
				while (true) {
					...
					//using (var reader = new StreamReader (stream)) 
					{
						...
						byte [] mainBuffer = new byte [100000];
						...
						while (true) {
							...
							await stream.ReadAsync (buffer, 0, 1).WithCancellation (cancel).ConfigureAwait (false);
							mainBuffer [mainBufferCount++] = buffer [0];

							if (mainBufferCount > 0 &&
								mainBuffer [mainBufferCount - 1] == 10) {
								line = System.Text.Encoding.UTF8.GetString (mainBuffer, 0, mainBufferCount);
								mainBufferCount = 0;
							}
